...
### 永久排除：硬性门槛（学历 / 技能）— 2026-07-30 新增
> 用户原话：「明确要求 python 的岗去掉，如果是会 python『优先』就不用去掉；公众号发布的国企/事业单位且要求硕士的去掉，私企要求硕士不用去掉」
> 背景：用户为本科、技能为 SQL/PowerBI/Excel，**无硕士学历、不会 Python**。此前匹配算法只比岗位名/关键词，给出 90%+ 高分却投不了，用户已手动标了一批「不匹配」，现固化为自动规则。

- **规则 1（技能）**：岗位**明确要求 Python（必需）** → 直接去掉（不收录）；若仅写「Python 优先 / 熟悉 Python 者优先」→ **保留**（不算必需）。
- **规则 2（学历·分主体）**：岗位属**国企 / 事业单位** 且**要求硕士（必需）** → 去掉；**私企**要求硕士 → **保留**（不去掉）。
- **实现位置**：`pipeline/matcher.py` → `MatchEngine.hard_exclude(job)`，由 `should_include(job, config)` 在 `update_manual.py` 预过滤阶段调用。
  - 公共部门判定 `is_public_sector()`：`type ∈ {state, gov, 国企}` 或 公司/来源含 `国资/国企/事业单位/事业编/研究院/局/委`。
  - 「必需 vs 优先」判定 `_mentions_without_pref()`：在关键词 ±22 字窗口内检索「优先 / prefer」，有则视为优先（保留），无则视为必需（去掉）。
- **验证（2026-07-30）**：全量 87 岗预演，硬过滤命中 17 个（Python 必需 7 + 国企/事业单位硕士 10）；**可见岗位 0 误伤**；**私企硕士 0 误删**；「Python 优先」类正确保留。
- **未来更新自动生效**：`update_manual.py` 合并种子岗位时，`should_include` 自动丢弃命中上述规则的岗位，无需每次手动标「不匹配」。
